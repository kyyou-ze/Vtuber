...
public class JniBridgeJava {
    ...
    /** Dipanggil dari Kotlin (MainActivity) tiap kali file audio TTS baru siap diputar. */
    public static native void nativeStartLipSync(String filePath);
    ...
    public static byte[] LoadFile(String filePath) {
        // Path absolut (mis. file wav hasil TTS di cache dir) dibaca langsung dari
        // filesystem. Path relatif (model, texture, dll) tetap dibaca dari assets
        // seperti semula.
        File f = new File(filePath);
        if (f.isAbsolute() && f.exists()) {
            try (InputStream fileData = new FileInputStream(f)) {
                int fileSize = (int) f.length();
                byte[] fileBuffer = new byte[fileSize];
                int totalRead = 0;
                while (totalRead < fileSize) {
                    int read = fileData.read(fileBuffer, totalRead, fileSize - totalRead);
                    if (read < 0) break;
                    totalRead += read;
                }
                return fileBuffer;
