...
void LAppModel::Update()
{
    ...
    // Lipsync real-time: dorong nilai RMS dari file wav TTS yang sedang diputar
    // ke parameter ParamMouthOpenY (atau apapun yang dideklarasikan model3.json).
    if (_wavFileHandler.Update(deltaTimeSeconds))
    {
        for (csmInt32 i = 0; i < _lipSyncIds.GetSize(); ++i)
        {
            _model->AddParameterValue(_lipSyncIds[i], _wavFileHandler.GetRms(), 0.8f);
        }
    }
